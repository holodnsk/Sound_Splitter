...
for filename in sorted(os.listdir()):
    ...
    print(
        "File {} is going to be split into several separate files. ".format(
            filename))
    ...
    while (remaining_chunk.duration_seconds > SIZE_LIMIT):
        ...
        while len(index) < 3:
            index = str(0)+index
        save_song(first_chunk, "{0}-{1}.{2}".format("out\\"+filename.replace(" ","_"), index, OUTPUT_FORMAT))
        size_check = size_check + len(first_chunk)/1000
        i += 1
    index = str(i)
    while len(index) < 3:
        index = str(0) + index
    save_song(remaining_chunk, "{0}-{1}.{2}".format("out\\"+filename.replace(" ", "_"), index, OUTPUT_FORMAT))
